...
class Dia_Extras_Admin {
	...
	function initialize() {
		...
		$debug->log( __( 'Plugin Loaded', DIA_TEXTDOMAIN ) );
		/*
		 * Load Wp_Admin_Notice for the notices in the backend
		 * 
		 * First parameter the HTML, the second is the css class
		 */
		new WP_Admin_Notice( __( 'Updated Messages', DIA_TEXTDOMAIN ), 'updated' );
		new WP_Admin_Notice( __( 'Error Messages', DIA_TEXTDOMAIN ), 'error' );
		/*
		 * Dismissible notice
		 */
		dnh_register_notice( 'my_demo_notice', 'updated', __( 'This is my dismissible notice', DIA_TEXTDOMAIN ) );
		/*
		 * Review Me notice
		 */
		new WP_Review_Me( array(
			...
			'slug' => DIA_TEXTDOMAIN,
			'rating' => 5,
			'message' => __( 'Review me!', DIA_TEXTDOMAIN ),
			'link_label' => __( 'Click here to review', DIA_TEXTDOMAIN )
				) );
		/*
		 * Load CronPlus 
		 */
		$args = array(
			...
			'plugin_root_file' => 'display-ids-advanced.php'
		);
		...
		$post_columns->add_column( 'cmb2_field', array(
			'label' => __( 'CMB2 Field' ),
			'type' => 'post_meta',
			'meta_key' => '_demo_' . DIA_TEXTDOMAIN . '_text',
			'orderby' => 'meta_value',
			'sortable' => true,
			'prefix' => '<b>',
			'suffix' => '</b>',
		...
		new Yoast_I18n_WordPressOrg_v3(
				array(
			'textdomain' => DIA_TEXTDOMAIN,
			'display_ids_advanced' => DIA_NAME,
			'hook' => 'admin_notices',
				)
		);
		...
		$plugin = Display_IDs_Advanced::get_instance();
	 ...
	 * 
	 * DIA_Extras->web_push_notification( 'Title', 'Content', 'http://domain.tld');
	 * 
...
$dia_extras_admin = new Dia_Extras_Admin();
$dia_extras_admin->initialize();
do_action( 'display_ids_advanced_dia_extras_admin_instance', $dia_extras_admin );
